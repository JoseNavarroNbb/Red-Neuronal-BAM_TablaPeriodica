...
He = np.array([-1, -1, -1, -1, -1, -1]).reshape(6, 1) 
Li = np.array([1, 1, -1, -1, 1, 1]).reshape(6, 1) 
Be = np.array([-1, -1, 1, 1, -1, -1]).reshape(6, 1) 
...
Ne = np.array([-1, -1, -1, -1, 1, 1]).reshape(6,1)
  
# Conjunto Y: Patrón de destino
h1 = np.array([1, 1, 1, 1]).reshape(4, 1) 
he1 = np.array([-1, -1, -1, -1]).reshape(4, 1) 
li1 = np.array([1, -1, 1, -1]).reshape(4, 1) 
be1 = np.array([-1, 1, -1, 1]).reshape(4, 1)
...
ne1 = np.array([-1, -1, 1, -1]).reshape(4, 1)
 

# Calcular la matriz de pesos: W
x = np.concatenate((H, He, Li, Be, B, C, N, O, F, Ne), axis = 1) 
y = np.concatenate((h1.T, he1.T, li1.T, be1.T, b1.T, c1.T, n1.T, o1.T, f1.T, ne1.T), axis = 0) 
print("\nMatriz de pesos:") 
